#include "KdResorcefactory.h"

// １回読み込んでいたら２回目以降は使いまわすようにする
std::shared_ptr<KdModel> KdResorceFactory::GetModel(const std::string& filename)
{
	// filenameのモデルがあるか？
...
// テクスチャデータ所得
std::shared_ptr<KdTexture> KdResorceFactory::GetTexture(const std::string& filename)
{
	// filenameのテクスチャがあるか？
...
json11::Json KdResorceFactory::GetJSON(const std::string& filename)
{
	// filenameの物があるか？(検索)
...
// JSON読み込み
json11::Json KdResorceFactory::LoadJSON(const std::string& filename)
{
	// Jsonファイルを開く
