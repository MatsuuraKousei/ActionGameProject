...
public:
	...
	// アニメーションの分割数を設定
	// 今は関数でセットするけどJsonとかで設定するようにしたほうがいい
...
private:
	// アニメーション関連
	int		m_animSplitX = 1;	// 縦の分割数
	int		m_animSplitY = 1;	// 横の分割数

	float	m_animPos = 0;		// 現在のコマの位置(floatじゃないと細かいspeedに対応できない)
	...
	std::shared_ptr<KdTexture> m_texture;	// 張り付けるテクスチャ情報
};